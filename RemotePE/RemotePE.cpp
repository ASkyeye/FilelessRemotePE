...
void PELoader(char* data, const long long datasize)
{
    ...
    pImageBase = (BYTE*)VirtualAlloc(preferAddr, ntHeader->OptionalHeader.SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!pImageBase) {
        if (!relocDir) {
            exit(0);
        }
        else {
            pImageBase = (BYTE*)VirtualAlloc(NULL, ntHeader->OptionalHeader.SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
            if (!pImageBase)
            {
                exit(0);
            }
    ...
    // No New Thread :
    //((void(*)())retAddr)(); // bad
    VirtualProtect(preferAddr, ntHeader->OptionalHeader.SizeOfImage, PAGE_EXECUTE_READWRITE, &OldProtect);
    EnumThreadWindows(0, (WNDENUMPROC)retAddr, 0);
    VirtualProtect(preferAddr, ntHeader->OptionalHeader.SizeOfImage, PAGE_READWRITE, &OldProtect);
